      ...
      subcommand
        ...
        .addStringOption((option) =>
          option
            .setName("song")
            .setDescription(
              "Song artist and title, or a YouTube, Soundcloud, or Spotify playlist URL"
            )
            .setRequired(true)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("pause").setDescription("Pauses the current track")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("remove")
        ...
        .addStringOption((option) =>
          option
            .setName("number")
            .setDescription(
              "Use /listen queue to find the queue number you want to remove from queue"
            )
            .setRequired(true)
        )
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("skip").setDescription("Skips the song playing")
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("unpause").setDescription("Resumes the audio player")
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("stop")
  ...
  async execute(interaction, memory) {
    ...
    if (!serverContruct.voiceChannel || !serverContruct.player) {
      ...
      serverContruct.player.on(AudioPlayerStatus.Idle, async (data) => {
        ...
        serverContruct = memory.setServerQueue(
        ...
        if (!serverContruct.songs.length) {
          ...
          serverContruct = {};
          return (serverContruct = memory.setServerConnection(
            interaction.guild.id,
            { songs: [] },
            true
          ));
    ...
    switch (interaction.options.getSubcommand()) {
      ...
      case "play":
        ...
        if (!songURL) {
          return interaction.editReply(
            "There was no song identified in your command. Please try again."
          );
        }
        ...
        if (songURL.includes("youtube.com") || songURL.includes("youtu.be")) {
          ...
          songURL.includes("soundcloud.app")
        ) {
        ...
        if (Array.isArray(song)) {
          ...
          song.map((s) => {
            content += `\n- ${s.title}`;
            serverContruct = memory.appendToServerQueue(
              interaction.guild.id,
              s
            );
          });
          ...
          if (youtubeURL || soundcloudURL) {
            if (serverContruct.songs.length) {
              ...
              console.log(
                `${interaction.guild.id || guild.id}: Queued ${song.title}`
              );
              return interaction.channel.send({ embeds: [embed] });
            }
            serverContruct = memory.appendToServerQueue(
      ...
      case "queue":
        ...
        serverContruct.textChannel.send({ embeds: [embed] });
        return (serverContruct = memory.setServerQueue(interaction.guild.id, [
          currentSong,
          ...serverContruct.songs,
        ]));
        break;
      ...
      case "remove":
        ...
        if (!songInt || typeof songInt !== "number" || songInt <= 0) {
          return interaction.editReply(
            "Gibson audio player can not remove that songId"
          );
        }
      ...
      case "shuffle":
        ...
        return (serverContruct = memory.setServerQueue(
          interaction.guild.id,
          serverContruct.songs
        ));
        break;
...
const play = async (serverContruct, guild, song, memory) => {
  try {
    ...
    if (song.type === "youtube") {
      ...
      const scStream = await scdl
        .download(song.url, scClientId)
        .then((stream) => (resource = createAudioResource(stream)));
    }
    await new Promise((done) => setTimeout(done, 3550));
    serverContruct.player.play(resource, {
      inputType: type,
      highWaterMark: 1 << 25,
    ...
    serverContruct.textChannel.send(
      `There was an issue processing ${song.title}: ${song.url}`
    );
    console.log(
      `${guild.id}: There was an issue processing ${song.title}: ${song.url}:\n`,
      err
    );
